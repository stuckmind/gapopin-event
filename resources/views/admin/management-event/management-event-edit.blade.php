...
<div class="bg-white p-6 space-y-6">
    ...
    <form action="{{ route('management-event.update', $event->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div>
            <label class="block text-gray-700 font-semibold mb-2">Name Tag</label>
            <input type="file" name="file_name_tag" class="w-full py-2 px-4 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
            @if ($event->file_name_tag)
                <div class="mt-3">
                    <p class="text-gray-600 text-sm mb-1">Name Tag Event Saat Ini:</p>
                    <img src="{{ asset('storage/' . $event->file_name_tag) }}" class="w-32 h-32 object-cover rounded-md shadow">
                </div>
            @endif
            @error('file_name_tag') <p class="text-red-500 text-sm">{{ $message }}</p> @enderror
        </div>
